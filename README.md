...
> **Agent 镜像两个变体**：
> - `opencli-admin-agent:0.3.2` — 默认，约 200 MB，通过 `host.docker.internal` 连接宿主机 Chrome
> - `opencli-admin-agent:0.3.2-chrome` — 约 1.2 GB，内置 Chromium，完全自包含
...
镜像已发布至 Docker Hub（`xjh1994/opencli-admin-{api,frontend,agent}:0.3.2`），无需本地构建。从源码构建：

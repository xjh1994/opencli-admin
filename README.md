...
- **数据看板** — 7 天任务执行趋势折线图、新增采集量柱状图、StatCard 今日 vs 昨日趋势角标、最近执行记录一览
- **数据源管理** — 支持 opencli、RSS、API、Web 爬虫、CLI 五种渠道类型；卡片 Grid 布局，名称搜索 + 类型筛选 chip，一键测试连通性，状态快速切换
- **定时计划** — 结构化频率设置（每 N 分钟 / 每小时 / 每天 / 每周 / 每月 / 指定时间），支持时区和一次性执行；可为每条计划指定专用 Agent 实例
- **采集任务** — 实时查看任务状态、执行历史、错误信息；手动触发时可临时指定 Agent 实例
- **采集记录** — 关键词搜索（模糊匹配 normalized_data / raw_data），点击行展开格式化 JSON + AI 分析，来源名称列，状态筛选、多选批量删除

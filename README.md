...
- **边缘节点（规划中）** — Agent 模式下，Chrome 运行在远程宿主机，通过 WebSocket 反向通道主动连接中心 API，中心无需直接访问边缘节点（适合 NAT/跨网部署）
...
每个 Chrome 实例在「浏览器管理」新增时可指定节点类型：

| 类型 | 说明 | 状态 |
|------|------|------|
| **Local（本地）** | Chrome 运行在与 API 同一 Docker 网络，API 直接驱动，无需额外配置 | ✅ 默认 |
| **Agent（边缘节点）** | Chrome 运行在远程宿主机，Agent 进程主动 WebSocket 连接中心 API，中心无需直接访问边缘（适合 NAT/跨网） | 🚧 规划中 |

当前两种类型均走本地采集逻辑，`node_type` 字段已落库，为后续 Agent 分布式通道做好铺垫。
...
将输出的 `AGENT_POOL_ENDPOINTS` 写入 `.env`，重启 API 即生效：
...
- 建议：扩容后先通过 noVNC 逐一确认各实例登录状态，再将 `AGENT_POOL_ENDPOINTS` 写入 `.env`
...
API 直接驱动同一 Docker 网络内的 Chrome 实例，零额外配置，适合单机部署。
...
分布式边缘采集：Chrome 实例运行在任意宿主机（可跨 NAT/内网），每个节点上的 opencli Agent 进程主动通过 WebSocket 反向通道连接中心 API，中心推送任务、节点返回结果。

```bash
COLLECTION_MODE=agent
```

**核心设计原则**：中心不主动访问 Agent（避免 NAT/防火墙问题），Agent 主动连接中心并保持长连接，中心通过已建立的连接推送任务。

实施路线：
1. **Phase 1（已完成）** — `node_type` 字段落库，`COLLECTION_MODE` 配置项，新增实例时可标记为边缘节点
2. **Phase 2（规划）** — WebSocket 反向通道 `/agents/ws`，中心维护在线 Agent 注册表
3. **Phase 3（规划）** — Agent 进程内嵌至 chrome 容器，`CENTRAL_API_URL` 配置自动上报
4. **Phase 4（规划）** — Celery 集成，跨机器任务分发
...
BrowserPool（单机：asyncio.Queue / 分布式：Redis BLPOP）
  ├─ chrome-1  (slot: 1, mode: bridge, node_type: local)
  ├─ chrome-2  (slot: 1, mode: cdp,    node_type: local)  ← 每实例同一时刻最多执行 1 个任务
  └─ chrome-N  (slot: 1, mode: ...,    node_type: agent)  ← 边缘节点（Phase 2+ 走反向通道）
    ↓
按 node_type 路由（当前 Phase 1：两者均走本地）
  ├─ local → 按 mode 直接驱动
  ...
  └─ agent  → WebSocket 反向通道推送任务（Phase 2+ 实现）
                └─ Agent 进程（chrome-N 宿主机）→ opencli 本地执行 → 结果回传中心
```
